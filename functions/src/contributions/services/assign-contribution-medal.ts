...
export const CONTRIBUTIONS_assignPlayerMissingContributions = functions.runWith({ timeoutSeconds: 540 }).pubsub.schedule('*/30 * * * *')
    .onRun(async (context) => {
        ...
        const unAwardedContributionsQuery = FirestoreInstance
            ...
            .orderBy(<keyof ContributionModel>'pubUserId')
        ...
        console.log(`unawarded: ${unAwardedContributions.size}`)
        let successful = 0;
        let unSuccessful = 0;
        for await (const snapshot of unAwardedContributions.docs) {
            ...
            const res = await awardContribution(batch, contribution, snapshot.ref);
            if (res) successful++;
            else unSuccessful++;
        }
        ...
        await batch.commit();
        console.log({ successfull: successful, unSuccessfull: unSuccessful })
    });
...
async function awardContribution(
    ...
    if (!playerPubCode) {
        console.error(`no pubcode found for https://lab-movil-2222.pubpub.org/user/${contribution.pubUserSlug}`);
        return false;
    }
