 ...
 */
export const CONTRIBUTIONS_updatePubWatchersFromProfile = functions.pubsub.schedule('*/30 * * * *')
    .onRun(async (data, context) => {
        ...
        const attributions = Object.values(userData?.attributions);
        ...
        const attributionsTasks = attributions.map(async (attribution: any) => {
            ...
            /// get collection to obtain the propperty of cityId
            ...
            const cityId = collectionsResponse.docs()[0].data().cityId;
